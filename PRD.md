...
### 3.10 Local Trial with Cloud Anti-Reset

LemonSqueezy doesn't support free trials on one-time purchase products, so the 14-day trial is managed entirely locally (stored in SQLite `settings` table). To prevent trial reset by reinstalling, the trial start date is also synced to Supabase's `device_trials` table. On launch, the app compares local and remote dates and uses the earlier one. If offline, it falls back to the local date. A 7-day offline grace period allows licensed users to work without network access.

### 3.11 Fail-Open License Checks

If the license status check fails entirely (network error + no cached state), the app lets the user through rather than blocking access. This prioritizes usability over enforcement — a musician mid-session shouldn't be locked out by a transient network issue. The `LicenseGate` component catches all errors and fails open.

### 3.12 Compile-Time LemonSqueezy Configuration

The LemonSqueezy checkout URL and API keys are injected via `option_env!()` at compile time (same pattern as Spotify/SoundCloud credentials). Dev builds use test-mode API keys; production builds use live keys. The checkout URL is baked into the binary so the "Buy License" button works without any runtime configuration.
...
| SmartScreen notice | Done | Amber callout in download modal warning about Windows SmartScreen + instructions to click "More info" → "Run anyway" |
| Deployment | Done | Auto-deploys from `skinut3232/setcrate-site` GitHub repo via Vercel |

### 4.10 License & Trial System (Beyond Spec - Complete)

| Feature | Status | Description |
|---------|--------|-------------|
| 14-day local trial | Done | Trial starts on first launch, countdown stored locally + synced to Supabase for anti-reset protection |
| LemonSqueezy activation | Done | License key activation via LemonSqueezy API (activate/validate/deactivate endpoints), 3-device limit |
| Offline grace period | Done | 7-day grace period allows full use without network after last successful validation |
| LicenseGate wrapper | Done | Top-level component blocks app access when trial expired or license invalid; fail-open on check failure |
| ActivationScreen | Done | Full-screen modal with license key input, "Activate" button, "Buy License" link to LemonSqueezy checkout |
| TrialBanner | Done | Persistent top bar during trial showing "X days left" + "Buy Now" button |
| LicenseSettings panel | Done | Settings section showing license status, masked key (last 4 chars), deactivate button, offline grace info |
| Dev mode bypass | Done | `#[cfg(debug_assertions)]` always returns `Activated` during `tauri dev` |
| Checkout URL command | Done | `get_checkout_url` Tauri command returns compiled LemonSqueezy checkout URL |
| LemonSqueezy webhook | Done | Supabase Edge Function receives webhook events (order_created, subscription_created/updated/expired, license_key_created) with HMAC-SHA256 signature verification |
| Webhook tables | Done | Supabase tables: `customers`, `orders`, `subscriptions`, `license_keys` with RLS policies scoped by email |

**License States:**

| Status | Meaning | User Access |
|--------|---------|-------------|
| `TrialActive` | Within 14-day trial | Full access, countdown banner shown |
| `TrialExpired` | Past 14 days, no license | Locked — activation screen only |
| `Activated` | Valid license key verified | Full access |
| `Expired` | License invalid/revoked/refunded | Locked — activation screen only |
| `OfflineGrace` | No network, within 7 days of last validation | Full access, warning badge |

**Pricing (LemonSqueezy):**
- Desktop: $29 one-time purchase (3-device activation limit)
- Mobile Sync: $3.99/month or $34.99/year subscription (1-device limit, managed by LemonSqueezy)

### 4.11 Additional Features (Beyond Spec)
...
| Right-click context menu | Done | Archive/unarchive, Open in Ableton, Open Project Folder — with keyboard nav, viewport boundary detection |
...
- Mobile Sync: 14-day trial → $3.99/month or $34.99/year subscription via LemonSqueezy
...
|---------|--------|----------|-------|
| Auto-updater | Planned (shelved) | Medium | Tauri v2 `tauri-plugin-updater` with Ed25519 signing, GitHub Releases `latest.json` endpoint, sidebar update indicator. Full plan designed but deferred — no code written yet. |
...
| **Missing `device_trials` Supabase table** | Medium | Low — Rust code references `device_trials` table for anti-reset sync, but no Supabase migration creates it yet; trial anti-reset only works locally |
| **README.md could be richer** | Low | Low — `README.md` exists but could include screenshots and more detailed setup instructions |

### 6.3 Future / Stretch Goals (From Spec Documents)

These are explicitly listed as future ideas in the spec documents, not current commitments:

- Auto-updater via `tauri-plugin-updater` (full design complete — Ed25519 signing, GitHub Releases endpoint, sidebar update indicator; shelved pending larger user base)
- Windows code signing certificate (EV/OV, ~$70-200/yr — deferred; SmartScreen reputation builds organically regardless of cert type since Microsoft's March 2024 policy change)
...
**Supabase-only tables** (via Edge Function webhook, not in local SQLite): `customers`, `orders`, `subscriptions`, `license_keys` — populated by LemonSqueezy webhook events for purchase/subscription lifecycle tracking.
