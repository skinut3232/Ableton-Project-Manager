    ...
    <div className="space-y-2">
      <div className="relative">
        <input
          ...
          autoFocus
          className="w-full rounded border border-border-default bg-bg-primary px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:border-green-500 focus:outline-none"
        />
        {search.isPending && (
          <div className="absolute right-3 top-1/2 -translate-y-1/2">
            <div className="h-4 w-4 animate-spin rounded-full border-2 border-text-muted border-t-green-500" />
          </div>
        )}
      </div>
      ...
      {results.length > 0 && (
        <div className="max-h-64 overflow-y-auto rounded-lg border border-border-default bg-bg-elevated/80">
          {tracks.length > 0 && (
            <>
              <div className="sticky top-0 bg-bg-elevated px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-text-muted border-b border-border-default">
                Tracks
              </div>
              {tracks.map((r) => (
...
function SearchResultRow({
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 hover:bg-bg-surface/50 transition-colors">
      {result.album_art_url ? (
        ...
        <div className="h-10 w-10 rounded bg-bg-surface shrink-0" />
      )}
      ...
      <button
        ...
        className={`shrink-0 rounded px-2 py-1 text-xs font-medium transition-colors ${
          isAdded
            ? 'bg-bg-surface text-text-muted cursor-default'
