...
export function CoverImage({ project, size, className = '', onClick, showLock }: CoverImageProps) {
  ...
  return (
    <div
      className={`relative rounded-${size === 'sm' ? '' : 'md'} bg-bg-surface overflow-hidden ${sizeClass} ${className} ${onClick ? 'cursor-pointer' : ''}`}
    ...
    >
      {project.artwork_path ? (
        ...
        <div className="flex items-center justify-center h-full">
          <span className={`text-text-muted ${size === 'sm' ? 'text-xs' : size === 'md' ? 'text-3xl' : 'text-4xl'}`}>
            &#9835;
          </span>
        </div>
      )}
      {showLock && project.cover_locked && (
        <div className="absolute top-1 right-1 bg-black/60 rounded-full p-0.5">
          <svg className="h-3 w-3 text-text-secondary" fill="currentColor" viewBox="0 0 20 20">
